function filterSensorsByDescription() {
  const sensorDescriptionTableHeader = document.getElementById(
    "sensorDescriptionTableHeader"
  );
  ...
  select.addEventListener("change", (e) => {
    ...
    //Remove filter from the table header and restore all previous html/css elements
      ...
      sensorDescriptionTableHeader.setAttribute(
        "onclick",
        "filterSensorsByDescription()"
      );
      ...
      removeAllChildNodes(tableBody[0]);
      createMainSensorTable(
        _store.sensors.data.payload.filter(
      ...
      //initialize Main Sensor Table ActiveRow Object
      _store.getSensorTableActiveRowObject(
        _store.sensors.data.payload.filter(
          (item) => item.description === e.target.value
        ).length
      );
